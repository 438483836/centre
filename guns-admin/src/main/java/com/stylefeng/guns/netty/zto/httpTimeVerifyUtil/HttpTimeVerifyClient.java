...
*/
public class HttpTimeVerifyClient {

    private static final Logger logger = LoggerFactory.getLogger(HttpTimeVerifyClient.class);
    ...
    public void start() {
        ...
        bootstrap.handler(new ChannelInitializer<SocketChannel>() {
            ...
            public void initChannel(SocketChannel ch) throws Exception {
                ...
                ch.pipeline().addLast(new HttpTimeVerifyClientHandler());
            }
        });
    ...
    public void sendImpl(String msg){
        ...
        channelFuture.channel().writeAndFlush(request).addListener((ChannelFutureListener) future -> {
            if (future.isSuccess()){
                logger.info("send time-verify request to zto success. msg:[{}]",msg);
            }else {
                logger.info("send time-verify request to zto fail. msg:[{}]",msg);
            }
        });
        try {
    ...
    public static void main(String[] args) throws Exception {
        HttpTimeVerifyClient client = new HttpTimeVerifyClient();
        client.start();
        client.sendImpl(HttpRequestFormat.timeVerify());
    }
}
